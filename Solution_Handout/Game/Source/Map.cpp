...
void Map::Draw()
{
    ...
    cameraItem = app->render->cameras.start;
            ...
            if (mapLayerItem->data->properties.GetProperty("Draw") != NULL && mapLayerItem->data->properties.GetProperty("Draw")->value) {
                ...
                for (int x = 0; x < mapLayerItem->data->width; x++)
                {
                    for (int y = 0; y < mapLayerItem->data->height; y++)
                    {
                        ...
                        if (app->render->IsOnCamera(pos.x, pos.y, mapData.tileWidth, mapData.tileHeight + 32, cameraItem->data))
                        {
                            app->render->DrawTexture(tileset->texture, pos.x, pos.y, cameraItem->data, &r);
                        }
                    }
                }
            }
