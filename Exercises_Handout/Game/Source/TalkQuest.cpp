...
bool TalkQuest::Update() {
	...
	ListItem<Player*>* playerItem = app->scene->player.start;
	while (playerItem != NULL)
	{

		if ((playerItem->data->tile.x * 32 >= npc->boundaries.x) &&
			(playerItem->data->tile.x * 32 < npc->boundaries.x + npc->boundaries.w) &&
			(playerItem->data->tile.y * 32 >= npc->boundaries.y) &&
			(playerItem->data->tile.y * 32 < npc->boundaries.y + npc->boundaries.h) &&
